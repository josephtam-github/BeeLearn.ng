...
if (isset($_POST["preview-page"])) {
	...
	if($check == false) {
		$errors["file-type"] = "<div class='py-1 text-white container-fluid bg-aqua text-center eAnimateLeft'>File is not an image </div>";
	} else {
		// Allow certain file formats
		if($file_type != "jpg" && $file_type != "png" && $file_type != "jpeg" && $file_type != "gif" ) {
			$errors["file-format"] = "<div class='py-1 text-white container-fluid bg-aqua text-center eAnimateLeft'>Only JPG, JPEG, PNG & GIF files are allowed </div>";
		} else {
			// Check file size
			if ($file_size > 50000000) {
			$errors["file-size"] = "<div class='py-1 text-white container-fluid bg-aqua text-center eAnimateLeft'>File is too large, it must be less than 5mb. </div>";
			} else {
				if (file_exists($target_name)) {
					$errors["file-exists"] = "<div class='py-1 text-white container-fluid bg-aqua text-center eAnimateLeft'>File already exists.</div>";
                } else {   
				    if ($file_error == 0) {
				    	if (empty($errors["file-type"] || $errors["file-format"] || $errors["file-size"] || $errors["file-exists"])) {
				    		...
				    		$errors["unknown-error"] = "<div class='py-1 text-white container-fluid bg-aqua text-center eAnimateLeft'>Something went wrong with the upload. Please try agan later. </div>";
				    	}
				    } else {
				    	$errors["unknown-error"] = "<div class='py-1 text-white container-fluid bg-aqua text-center eAnimateLeft'>Something went wrong <br> Please try agan later. </div>";
				    }
                }
			}
		}
				...
				if (file_exists($target_file)) {
					...
					if ($file_error == 0) {
						if (empty($errors["file-type"] || $errors["file-format"] || $errors["file-size"])) {
							if (move_uploaded_file($file_tmp_name, $target_file)) {
								$success["update"] = "<div class='py-1 text-white container-fluid bg-aqua text-center eAnimateLeft'> Your image has successfully been updated!</div>";
							} else {
								echo $userdb->error;
							}
						} else {
